...
public class Algorithm2 {
	...
	public static cpp.pii[][] BFS(int fromCol, int fromRow, int toCol, int toRow, cpp.pii block) throws PathBlockedException {
		...
		if (gm.towerManager.isWalkable(fromCol, fromRow) && (block.first != fromCol || block.second != fromRow)) {
		...
		while (!q.isEmpty()) {
			...
			for (int[] rc: Other.dir) {
				...
				if (nx < 0 || nx >= TABLE_C || ny < 0 || ny >= TABLE_R) { 
					continue;
				} else if (nt >= dist[nx][ny] || !gm.towerManager.isWalkable(nx, ny) || (nx == block.first && ny == block.second)) { // in bound but not shorted/						
					if (nt <= dist[nx][ny]  && (!gm.towerManager.isWalkable(nx, ny) || (nx == block.first && ny==block.second )) 
							&& nx != toCol && ny != toRow) { 
						dist[nx][ny] = nt;
