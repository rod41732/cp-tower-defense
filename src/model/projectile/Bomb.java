...
public class Bomb extends NormalProjectile {
	...
	public boolean collideWith(Monster m) {
		if (shouldCollide(m)) {
			...
			GameManager.getInstance().spawnParticle(new Particle(Images.explosion, impact.x, impact.second, 0, 0, 1000));
			System.out.println("Boom");
			for (Monster ms: GameManager.getInstance().getMonsters()) {
				if (Double.compare(ms.distanceTo(impact.x, impact.second), 3) < 0) {
